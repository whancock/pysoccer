...
def getTeamAvoidance(team, checkRange=50):
...
def getTeamNearestAvoidance(team, knob=20):
    currAgent = team[0]
    for agent in team:
...
def snapVectorHelp(num):

    inside = ((num + 180.) / 360) * 8
    inside = round(inside, 0)

    return ((inside / 8.) * 360) - 180


def snapVector(vector):
    return np.array([snapVectorHelp(num) for num in vector])
